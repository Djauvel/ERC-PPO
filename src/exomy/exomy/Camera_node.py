...
sys.path.append('/home/made/Documents/isaac_rover_physical-main/src/exomy/scripts/utils/ros2_numpy-foxy-devel')
import ros2_numpy
...
class Camera_node(Node):
    ...
    def callback(self, data_cam1, data_cam2):
        try:
            ...
            points[:,1]=pc1['y']
            points[:,2]=pc1['z']
            pcnp = np.array(points, dtype=np.float64)
            
            points2=np.zeros((pc2.shape[0],3))
            points2[:,0]=pc2['x']
            points2[:,1]=pc2['y']
            points2[:,2]=pc2['z']
            pcnp2 = np.array(points2, dtype=np.float64)
